'use client'

import Link from 'next/link'
import Image from 'next/image'
import { useState } from 'react'
import { useContent, cs, applyStyle, imgSrc, getAlign } from '@/hooks/useContent'

const S = 'hero'
const D = {
  accroche:  'Sophrologie & Coaching à Lyon, Giez (Proche Annecy) et visio',
  titre:     'Faire une pause pour mieux oser.',
  sousTitre: "Un espace doux et bienveillant pour vous reconnecter à vous-même, traverser les transitions de vie et révéler vos ressources profondes.",
}

export default function Hero() {
  const content = useContent()
  const [photoError, setPhotoError] = useState(false)

  const accroche  = cs(content, S, 'accroche',  D.accroche)
  const titre     = cs(content, S, 'titre',     D.titre)
  const sousTitre = cs(content, S, 'sousTitre', D.sousTitre)
  const photo     = cs(content, S, 'photo',     '')

  return (
    <section id="accueil" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-blanc-casse">
      <div className="absolute top-0 right-0 w-[40vw] h-[40vw] max-w-[600px] max-h-[600px] bg-vert-pastel/30 rounded-full -translate-y-1/3 translate-x-1/3 blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[30vw] h-[30vw] max-w-[400px] max-h-[400px] bg-rose-pastel/30 rounded-full translate-y-1/3 -translate-x-1/4 blur-3xl pointer-events-none" />

      <div className="container-max section-padding relative z-10">

        {/* Accroche — alignement indépendant */}
        <p className="text-rose-saumon text-sm font-medium tracking-widest uppercase mb-6"
           style={{ ...applyStyle(content, S, 'accroche'), ...getAlign(content, S, 'accroche') }}>
          {accroche}
        </p>

        {/* Titre — alignement indépendant */}
        <h1 className="text-5xl md:text-7xl lg:text-8xl text-texte leading-tight mb-6"
            style={{ ...applyStyle(content, S, 'titre'), ...getAlign(content, S, 'titre') }}>
          {titre}
        </h1>

        {/* Sous-titre — alignement indépendant */}
        <p className="text-lg md:text-xl text-texte/70 max-w-2xl mx-auto mb-8 leading-relaxed"
           style={{ ...applyStyle(content, S, 'sousTitre'), ...getAlign(content, S, 'sousTitre') }}>
          {sousTitre}
        </p>

        {/* Photo optionnelle entre le texte et les boutons */}
        {photo && !photoError && (
          <div className="flex justify-center mb-10">
            <div className="relative w-64 h-64 md:w-80 md:h-80 rounded-3xl overflow-hidden shadow-lg">
              <Image src={imgSrc(photo)} alt="Photo hero" fill className="object-cover"
                sizes="(max-width: 768px) 256px, 320px"
                onError={() => setPhotoError(true)} />
            </div>
          </div>
        )}

        <div className="flex flex-col sm:flex-row gap-4 items-center sm:justify-center">
          <Link href="/rdv" className="btn-primary">Prendre rendez-vous</Link>
          <a href="#vision" className="btn-outline">Découvrir l&apos;approche</a>
        </div>

        <a href="#qui-suis-je" aria-label="Défiler vers le bas"
          className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 text-texte/30 hover:text-rose-saumon transition-colors">
          <svg className="w-8 h-8 animate-bounce" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
          </svg>
        </a>
      </div>
    </section>
  )
}
